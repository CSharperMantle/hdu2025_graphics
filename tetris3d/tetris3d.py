...
game_center = (
    game.dims[0] / 2.0,  # Xm -> Xg
    game.dims[2] / 2.0,  # Zm -> Yg
    game.dims[1] / 2.0,  # Ym -> Zg
)
...
def repose_camera():
    gl.glLoadIdentity()
    eye_x = game_center[0] + camera_distance * math.cos(
        math.radians(camera_pitch)
    ) * math.sin(math.radians(camera_yaw))
    eye_y = game_center[1] + camera_distance * math.sin(math.radians(camera_pitch))
    eye_z = game_center[2] + camera_distance * math.cos(
        math.radians(camera_pitch)
    ) * math.cos(math.radians(camera_yaw))
    glu.gluLookAt(
        eye_x,
        eye_y,
        eye_z,
